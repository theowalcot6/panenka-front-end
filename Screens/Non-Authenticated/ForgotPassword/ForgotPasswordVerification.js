...
export default function ForgotPasswordVerification({ navigation, route }) {
  ...
  const handleNext = async () => {
    ...
    try {
      const response = await axios.post(`${API_URL}/users/verify-code`, {
        email: email,
        code: code, // Added the code to the request payload
      });

      if (response.status === 200) {
        navigation.navigate('ForgotPasswordPassword', { email: email, code: code });
